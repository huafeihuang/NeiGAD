...
from . import DeepDetector,Detector
...
class VGOD(DeepDetector):
    ...
    def fit(self, data, y_true=None):
        ...
        for epoch in range(self.epoch):
            ...
            if self.verbose:
                print("Epoch {:04d}: Loss {:.4f}"
                      .format(epoch, epoch_loss / data.x.shape[0]), end='')
        ...
        # self.decision_score_ = decision_scores.detach().cpu().numpy()
        self.decision_score_ = decision_scores.detach().cpu()
    ...
    def fit_with_fairness(self, data, y_true=None, fair_factor=0.1, sens_var_col=None, adcg_factor=0.01,
        ...
        # self.decision_score_ = score.detach().cpu().numpy()
        self.decision_score_ = score.detach().cpu()

        # self._process_decision_scores()
        self._process_decision_score()
        return self

    def decision_function(self, G, label=None):
        # outlier_scores = self.decision_scores_
        outlier_scores = self.decision_score_
    ...
    def _process_decision_score(self):
        ...
        #     'int').ravel()
        # self.threshold_ = percentile(self.decision_score_,
        #                              100 * (1 - self.contamination))
        # self.label_ = (self.decision_score_ > self.threshold_).astype(
        #     'int').ravel()
        ...
        self._mu = self.decision_score_.mean()
        self._sigma = self.decision_score_.std()
        return
# init_model, forward_model
